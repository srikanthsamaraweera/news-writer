...
export const TopicCard: React.FC<TopicCardProps> = ({ topic, index, selectedModel }) => {
  ...
  const handleCopyHtml = () => {
    if (detailedSummary) {
      navigator.clipboard.writeText(detailedSummary).then(() => {
        setCopySuccess(true);
        setTimeout(() => setCopySuccess(false), 2500); // Reset after 2.5 seconds
      }).catch(err => {
      ...
      <div className="flex items-start space-x-4">
        ...
        <div className="flex-1">
          ...
          {detailedSummary ? (
             ...
             <p className="text-slate-300 leading-relaxed min-h-[4em]">
                {topic.summary}
            </p>
          )}
